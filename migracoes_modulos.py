#!/usr/bin/env python3
"""
Migração para adicionar sistema modular e módulo de manutenção
Execute: python3.10 migracoes_modulos.py
"""
...
def criar_modulos_iniciais():
    ...
    with app.app_context():
        ...
        result = db.session.execute("SELECT COUNT(*) FROM modulos").fetchone()
        if result[0] > 0:
...
def ativar_modulos_tenant():
    ...
    with app.app_context():
        ...
        # Buscar módulos
        modulos = db.session.execute("SELECT id, slug FROM modulos WHERE ativo = 1").fetchall()
